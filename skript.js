...
function lisaKorviJalus() {
    ...
    `);
        ...
        document.querySelector('[data-action="kassa"]').addEventListener('click', () => alustaTaimer(120,document.getElementById("time")));
    }
}
...
const sooduskood = document.getElementById("sooduskood");
...
form.addEventListener("submit", (e) => {
    ...
    else if (/\d/.test(eesnimi.value.trim())){
    ...
    else if (/\d/.test(perenimi.value.trim())){
        errors.push("Perenimi sisaldab numbreid")
    }

    if (telefon.value.trim().length < 6) {
        errors.push("Telefoninumber on lühem kui 6 sümbolit")
    }
    if (/\D/.test(telefon.value.trim())) {
        errors.push("Telefoninumber peab sisaldama vaid numbreid")
    }
    ...
    if (!tarne1.checked && !tarne2.checked) {
        errors.push("Üks raadionuppudest peab olema valitud");
    }

    // minu kood
    if (sooduskood.value.trim() !== "" && sooduskood.value.trim() !== "ÜLESANNE5") {
        errors.push("Sellist sooduskoodi ei eksisteeri");
    }
